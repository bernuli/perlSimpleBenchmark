...
#### Make strong suggestion for perl 5.18

## Since perl versions vary in loop performance, lets try to stick with 5.18
unless ($^V =~ /^v5.18/) {
...
my @passTimes; $passTimes[0]=["0","0"];  ## Array of array.  For each pass store the number of threads and time to complete.
...
my $psbRoot            = "$ENV{'HOME'}/psb";  ## This is where we will write CSV file.
...
#$numberofInnerLoops    = 10000000;  ## for testing
...
my $benchmarkEngineVersion = perlSimpleBenchmarkEngine_0_3_0();  ## with no args returns version.

## Start with 1 thread and increase by 1 thread each time.
for my $i (1 .. $numberOfOuterLoops) {

	print "\nStarting pass $i of $numberOfOuterLoops.\n";
	my ($forks, $forkedTime) = perlSimpleBenchmarkEngine_0_3_0($i);
	push @passTimes, [$forks, $forkedTime];

	print "Pass $i complete. $forks thread(s), $forkedTime seconds.\n";

	outputPassTotals();

}

## Do one more pass with a single thread.
## This may be not worth while, will leave commented for now.
# print "One more pass with single thread running\n";
# my ($forks, $forkedTime) = perlSimpleBenchmarkEngine_0_3_0(1);
# push @passTimes, [$forks, $forkedTime];
# print "Done. $forks, $forkedTime, $benchmarkEngineVersion\n";
# outputPassTotals();
...
## Script Version Info
push @rowData,  "$scriptName $scriptVersion benchmarkEngineVersion $benchmarkEngineVersion  Loop used:    for (0 .. $numberofInnerLoops ) {}";
...
for my $i (1 .. 2) {
	...
	## Straight loop times.
	if ($i == 1) {
		for my $ii (1 .. $#passTimes) {
			#my ($threads, $time) = split / /,$passTimes[$ii],2;

			print FHO $passTimes[$ii][1];

			## If the is the last pass, then add note about 1 thread in time field
			if ($ii eq $#passTimes) {
				print FHO " (only one thread)";
			}
	...
	## Delta loop times.
 	if ($i == 2) {
 		for my $ii (1 .. $#passTimes) {
 			#my ($threads, $time) = split / /,$passTimes[$ii],2;
 			my $delta = $passTimes[$ii][1];
  			if ($ii > 1) { $delta = $passTimes[$ii][1]-$passTimes[$ii-1][1]; }
 			print FHO "$delta" . ",";
 		}
...
sub perlSimpleBenchmarkEngine_0_3_0 {

my $benchmarkEngineVersion = "0.3.0"; ## Now in subroutine version.
return $benchmarkEngineVersion unless $_[0];

my $threads = $_[0];


#### BENCHMARK BELOW (bench V 0.3.0)
###
##
#
	...
	for my $ii (1 .. $threads) {
		...
		if ($pid == 0) {
			...
			print "\tFork $ii time: $innerLoopElapsedTime\n";
			exit;		
		}
		push @pids, $pid;
	...
	my $forkedTime = 999999999;
	$forkedTime = (gettimeofday - $outerStartTime);

return ($threads, $forkedTime);
}


sub outputPassTotals {
	## Output some status info before going on to next outer loop.
	print "\nPASS TOTALS:\n";
 	for my $i ( 1 .. $#passTimes ) {
 		print "Pass $i: $passTimes[$i][0] threads, $passTimes[$i][1] seconds\n";
 	}
}
